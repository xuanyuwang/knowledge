...
func TestReview7402ExportOverlapSQL(t *testing.T){
 ...
 for _,tc:=range cases{t.Run(tc.name,func(t *testing.T){
  ...
  if tc.name=="empty-oneof" { if err==nil { t.Fatal("empty oneof accepted") }; return }; if err!=nil{t.Fatal(err)}
  ...
  if err:=os.WriteFile("/tmp/convi7402-fixed/generated-"+tc.name+".sql",[]byte(combineQueryAndArgs(query,args)),0600);err!=nil{t.Fatal(err)}
...
func TestReview7402SubmitTimeSQL(t *testing.T) {
 ...
 for _,kind:=range []string{"conversations","stats"} {
  ...
  var momentTR *analyticspb.TimeRange
  ...
  if err:=os.WriteFile("/tmp/convi7402-fixed/submit-"+kind+".sql",[]byte(combineQueryAndArgs(query,args)),0600);err!=nil{t.Fatal(err)}
 }
}
