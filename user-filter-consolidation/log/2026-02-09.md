...
Created `implementation-considerations.md` covering 12 considerations for building a reliable shared user filter:

1. **No regression** — behavioral test suite first + shadow mode comparison
2. **Smooth transition** — new code alongside old, migrate callers one-by-one
3. **Small PRs** — natural boundaries: tests, types, implementation, per-caller migration, cleanup
4. **Feature flags** — proto-based config (per-customer, runtime-updatable) over current env var
5. **Debuggability** — structured logging at decision points, Datadog metrics, duration tracking
6. **Rollback strategy** — per-customer flag disable (instant), global disable, PR revert
7. **Shadow mode** — run both paths, compare, log differences, return old result until validated
8. **Performance** — baseline measurement, benchmark tests, memory profiling
9. **API ergonomics** — options struct instead of 15+ positional params, constructor-injected deps
10. **Testing strategy** — unit tests per behavior ID, comparison tests per caller, optional integration
11. **Documentation** — godoc, migration guide, behavioral standard as spec
12. **Monitoring** — permanent metrics, alerts for error/latency/early-return spikes

Also outlined 7 implementation phases: behavioral tests → types → implementation → config flag → shadow mode → per-caller migration (29 PRs) → cleanup.

### Detailed Implementation Plan

Created `implementation-plan.md` with:

**Caller inventory**: 30 total callers enumerated — 12 on ParseUserFilterForAnalytics, 15 on old pattern, 3 coaching. Each with file path, function name, and caller pattern classification.

**6 phases, ~47 PRs total**:
- Phase 1 (4 PRs): Behavioral test suite — ACL+base population, selection filtering, group mechanics, edge cases
- Phase 2 (2 PRs): New types/interface in shared/user-filter/ (ParseOptions, ParseResult, LiteUser, LiteGroup, Parser struct, utilities)
- Phase 3 (4 PRs): Unified implementation — core parse, selection filtering, group membership tracking, retarget tests
- Phase 4 (2 PRs): Proto config flag (field 21 in insights.proto) + shadow mode comparison helper
- Phase 5 (30 PRs): Per-caller migration in 5 waves — time-range (6), agent leaderboard (18), team leaderboard (2), special (1), coaching (3)
- Phase 6 (5 PRs): Cleanup — remove ParseUserFilterForAnalytics, old inline pattern, old Parse interface, feature flag, env var

**Rollout strategy**: Per-wave flag enablement — 1 test customer → 5 customers → all customers → next wave. 2+ sprints stability before cleanup.

**Open questions**: Shadow mode scope, coaching caller timeline (ListUsers vs ListUsersForAnalytics), PR batching for identical-pattern callers.

### Open Questions Resolved + Coaching Investigation

Resolved all 4 open questions:

1. **Shadow mode**: 1 caller per pattern — retrieve_note_taking_stats (time-range), retrieve_agent_stats (agent leaderboard), retrieve_knowledge_base_stats (team leaderboard), 1 coaching caller.
2. **Coaching callers**: Migrate in same consolidation. Investigation found:
   - All 3 coaching callers only use `parsedFilter.UserNames` — no group maps
   - Coaching `ServiceImpl` has `UserServiceClient` but NOT `InternalUserServiceClient` — needs to be added
   - Coaching callers already have `profileID` but don't pass it to current `Parse()` — new `ParseOptions` includes it
   - `LiteUser` has all data coaching needs (user IDs + basic metadata)
   - Added prerequisite PR 5.12 to add `InternalUserServiceClient` to coaching module
3. **Who reviews**: Not a concern.
4. **Batching**: Identical-pattern callers batched. Phase 5 reduced from 30 PRs to 13. Total plan reduced from ~47 PRs to ~30.

### Phase 1: Behavioral Test Suite — Started

Began writing behavioral tests in `common_user_filter_test.go`. Added 22 new test cases across 4 categories (corresponding to PRs 1.1–1.4):

**Category 11: Behavioral ACL (B-ACL-*) — 2 tests**
- B-ACL-2: Limited access narrows explicit selections to managed set
- B-ACL-4: UNION with multiple groups and overlapping members

**Category 12: Behavioral Base Population (B-BP-*) — 3 tests**
- B-BP-1: Group expansion users filtered by base population (non-agent excluded)
- B-BP-3: Users outside base population excluded (phantom user)
- B-BP-4: Metadata sourced from base population, not group expansion (stale metadata scenario)

**Category 13: Behavioral Selection Filtering (B-SF-*) — 6 tests**
- B-SF-1: Empty selection with limited access returns all managed users
- B-SF-3: Combined user+group selections (documents Divergence 5: INTERSECTION behavior)
- B-SF-4: Partial selection silently drops non-existent users
- B-SF-5: Invalid user name with ACL enabled returns error
- B-SF-5: Invalid group name with ACL disabled returns error
- B-SF-6: User selection with root access still filters

**Category 14: Behavioral Group Mechanics (B-GS-*, B-GM-*, B-DU-*) — 5 tests**
- B-GS-4: Unparseable group name silently skipped during expansion
- B-GM-1: User-to-group mappings track direct memberships
- B-GM-3: Only TEAM groups in mappings (DYNAMIC_GROUP excluded)
- B-DU-1: Deactivated excluded with ACL and groups combined
- B-DU-2: Deactivated included when flag is false

**Category 15: Behavioral Query Opt & Sorting (B-SO-*, B-PG-*, B-GM-*, B-EH-*) — 6 tests**
- B-SO-2: All result slices sorted by Name (FinalUsers, GroupsToAggregate, UsersFromGroups)
- B-PG-2: Root group (UUID) excluded from mappings for team leaderboard
- B-PG-3: Default group (UUID) excluded from mappings for team leaderboard
- B-GM-5: hasAgentAsGroupByKey=true only includes direct memberships
- B-EH-4: Config client error propagated

**Total test suite: 54 tests (32 existing + 22 new), all passing.**
