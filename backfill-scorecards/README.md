...
13. **Process scorecards (template type=2) are NOT backfilled — no workflow supports it.** Both `reindex-conversations` (our backfill script) and `BackfillScorecardsWorkflow` discover scorecards by iterating over conversations (`conversation_id IS NOT NULL`). Process scorecards (`scorecard_templates.type=PROCESS`) have empty `conversation_id` and are never picked up. Additionally, `BackfillScorecardsWorkflow` hardcodes `isProcessScorecard=false` in `template_processor.go:441`. The ClickHouse write layer (`scorecard_score.go`) does support process scorecards via `findShardForProcessScorecard()` and `isProcessScorecard` flags, but no workflow passes `true`. Verified on Spirit: 100% of type=2 template scorecards (11,237 across 8 templates) have no `conversation_id`, vs 100% of type=1 (131,095) having one. This is a pre-existing pipeline gap, not a backfill issue.
