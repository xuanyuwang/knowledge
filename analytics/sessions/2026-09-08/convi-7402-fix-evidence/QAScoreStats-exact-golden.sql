	...
	,
		scorecard_score_per_conversation AS (
			SELECT
				conversation_id,
				scorecard_id,
				DATE_TRUNC('day', scorecard_time + toIntervalDay(0) + toIntervalHour(-10)) - toIntervalDay(0) - toIntervalHour(-10) AS truncated_time,
agent_user_id,
		...
		SUM(float_weight) FILTER (WHERE percentage_value >= 0) AS weight_sum
			...
			GROUP BY conversation_id, scorecard_id , truncated_time, agent_user_id
		)
		SELECT
