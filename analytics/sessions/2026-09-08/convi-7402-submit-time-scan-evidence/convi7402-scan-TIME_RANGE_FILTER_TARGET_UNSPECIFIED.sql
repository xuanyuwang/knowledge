		...
		t1_moment_annotation_filter_0 AS (
			...
			WHERE
				((conversation_start_time >= '2021-01-01 00:00:00') AND (conversation_start_time < '2021-01-31 00:00:00') AND (moment_template_id = 'include'))
		...
		t2_moment_annotation_filter_0 AS (
			...
			FROM
				moment_annotation_mv_by_metadata_d
			WHERE
				((conversation_start_time >= '2021-01-01 00:00:00') AND (conversation_start_time < '2021-01-31 00:00:00') AND (moment_template_id = 'include') AND (metadata_string_value = 'selected'))
		),
		...
		t1_moment_annotation_filter_1 AS (
			...
			FROM
				moment_annotation_mv_by_metadata_d
			WHERE
				((conversation_start_time >= '2021-01-01 00:00:00') AND (conversation_start_time < '2021-01-31 00:00:00') AND (moment_template_id = 'overlap'))
			GROUP BY conversation_id
		),
		t2_moment_annotation_filter_1 AS (
			...
			FROM
				moment_annotation_mv_by_metadata_d
			WHERE
				((conversation_start_time >= '2021-01-01 00:00:00') AND (conversation_start_time < '2021-01-31 00:00:00') AND (moment_template_id = 'overlap') AND (metadata_string_value = 'selected'))
		),
		moment_annotation_filter_1 AS (
			...
			moment_annotation_exclude_filter_0 AS (
				...
				FROM
					moment_annotation_mv_by_metadata_d
				WHERE
					((conversation_start_time >= '2021-01-01 00:00:00') AND (conversation_start_time < '2021-01-31 00:00:00') AND (moment_template_id = 'exclude'))
			),
			...
			scorecard AS (
				...
				FROM
					scorecard_d
					 WHERE ((scorecard_time >= '2021-01-01 00:00:00') AND (scorecard_time < '2021-01-31 00:00:00')) 
			),
			...
			scorecard_score AS (
				...
				FROM
					score_d
				WHERE
					((scorecard_time >= '2021-01-01 00:00:00') AND (scorecard_time < '2021-01-31 00:00:00') AND (percentage_value >= 0) AND (not_applicable <> true))
			)
