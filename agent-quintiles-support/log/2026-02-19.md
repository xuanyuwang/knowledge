...
**Solution:** True percentile-based quintiles. Rank all agents by score within each peer group (same time range, criterion, team, group), divide into 5 approximately equal groups:
...
- Rewrote `setQuintileRankForPerAgentScores(response)`: groups per-agent scores by non-agent dimensions (time range, criterion, team, group), sorts descending within each group, distributes into quintiles evenly via `assignQuintileRanks()`
- Both Postgres path (line 297) and ClickHouse path (line 707) call the same function
- 8 new unit tests: 10-agent even split, 3-agent small group, 7-agent uneven, independent criteria groups, non-agent rows untouched, all-same-score, single agent, nil safety
